...
% plot(World.Nodes.X(vehicle.plan), World.Nodes.Y(vehicle.plan), '-', 'Color', color, 'LineWidth', 0.5 );
...
plot([vehicle.x World.Nodes.X(vehicle.planHorizon(4:end))], [vehicle.y World.Nodes.Y(vehicle.planHorizon(4:end))], '-', 'Color', color, 'LineWidth', 1.5 );
for k = 1:length(conflicts)
...
% str = sprintf("v_r = %2.1f - ID = %d", vehicle.v_r, vehicle.ID);
